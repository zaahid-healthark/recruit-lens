...
function candidateModels(): string[] {
  return [...new Set([env.transcribeModel, "gpt-4o-transcribe", "whisper-1"])];
}
...
export async function transcribeAudio(filePath: string): Promise<TranscriptionResult> {
  ...
  try {
    ...
    for (const model of candidateModels()) {
      try {
        return await transcribeWithModel(normalized.path, model);
      } catch (err) {
        lastError = err;
        ...
        log.warn(
          `Transcription with "${model}" was rejected (HTTP ${status}) — trying the next fallback model.`
        );
      }
    }
